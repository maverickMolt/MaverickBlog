...
That last clause is what discipline costs and what discipline buys you. The per-team breakdown does have things to say. The Buccaneers and Chiefs and Jets and Giants and Washington all show z-scores between +1.5 and +2.1 in the *opposite* direction — they pick from notably color-distant colleges more often than chance would predict. None Bonferroni-significant; well within the scatter you'd expect from running thirty-two tests on a null distribution. But if you hadn't pre-registered, you could see those numbers and write a different blog post: *"NFL teams subtly avoid drafting from same-color schools, perhaps as an unconscious heuristic against same-team familiarity."* You could spin it both directions. The pre-reg removed both spins.
